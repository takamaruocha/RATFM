...
# Generate aligned reconstruction sequences using best-matching training subsequences
def concatenate_sequences(target_file, candidate_files, folder, context_length, forecast_horizon):
    ...
    for start in range(1120 - (context_length - forecast_horizon), len(target_data) - context_length + 1, stride):
        seq_start = start
        seq_end = seq_start + context_length
        reconst_start = seq_end - forecast_horizon
        ...
        t_label = target_labels[reconst_start:seq_end]
...
def convert_to_jsonl(domain, domain_dict, folder, output_file, context_length, forecast_horizon):
    ...
    candidate_files = find_matching_files(domain_dict, domain, folder)
    ...
    with open(output_file, 'w', encoding='utf-8') as out:
        for target_file in candidate_files:
            ...
            best_match_files = [f for f in candidate_files if f != target_file]
            if not best_match_files:
                print(f"No other matching files found for: {target_file}")
                continue

            sequences = concatenate_sequences(target_file, best_match_files, folder, context_length, forecast_horizon)
