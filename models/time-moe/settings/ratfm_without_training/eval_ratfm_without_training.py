...
from utils.data_loader_ratfm import get_loader
...
for domain, entities in domain_dict.items():
    ...
    for target_line_number, entity in enumerate(entities):
        ...
        if matching_files in excluded_files:
            continue
  
        dataset_path = os.path.join(ucr_root_path, matching_files)
        if not os.path.exists(dataset_path):
            continue

        print(f"Processing dataset: {matching_files}")

        # Detect period using training dataset
        train_series = get_time_series_period(
            root_path=ucr_root_path,
            dataset=matching_files,
            scale=True
        )
        ...
        print(f"Completed: {matching_files}\n")
