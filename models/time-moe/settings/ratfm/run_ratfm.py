...
for domain in domain_dict.keys():
    domain_data_path = os.path.join(base_data_path, f"{domain}.jsonl")
    print(f"[{domain}] Checking data path: {domain_data_path} (exists: {os.path.exists(domain_data_path)})")

    # Define output directory
    output_dir = os.path.join(save_base, domain)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        print(f"[{domain}] Created output directory: {output_dir}")
    ...
    print(f"[{domain}] Executing: {' '.join(command)}")
    try:
        subprocess.run(command, check=True)
    except subprocess.CalledProcessError as e:
        print(f"[{domain}] Error during execution: {e}")
